...
def setup_argparse():
    parser = argparse.ArgumentParser(description='Complete the last sentence with 1-4 appropriate words')
...
def get_model_response(client, text):
    try:
        ...
        prompt = f"""
You are tasked with completing a sentence by adding 1-4 NEW, DIFFERENT words.

RULES:
1. ONLY provide 1-4 NEW words that complete the sentence naturally
...
Your completion (ONLY 1-4 NEW words):"""
        ...
        response = client.chat.completions.create(
            ...
            top_p=TOP_P,
            max_tokens=20  # Limit to very few tokens for short completions
        )
        
        completion = response.choices[0].message.content.strip()
        debug_print(f"Completion received: {completion}")
        
        # Ensure we only return 1-4 words
        words = completion.split()
        if len(words) > 4:
            debug_print(f"Trimming completion to 4 words")
            completion = " ".join(words[:4])
...
def clean_segment(content, window=None):
    """
    Complete the last sentence of the provided text with 1-4 appropriate words.

...
# How to use: python3 ts4.py inputfile.txt
...
def setup_logging():
    ...
    logging.basicConfig(
        filename='/home/j/Desktop/code/notepadMOD/logs/ts4.log',
        filemode='w',
        level=logging.INFO,
...
def clean_segment(segment):
    ...
    try:
        ...
        instruction = """
Refine and polish the following text while maintaining its core message and meaning. The refinement should:

1) Improve clarity and readability while keeping the original meaning intact
2) Maintain the original tone and intent of the message
3) Keep any specific details, technical terms, or key points
4) Preserve the general structure and format of the text
5) Remove redundancy while keeping important emphasis
6) Ensure proper grammar and punctuation
7) Keep the text concise but impactful

Please refine the following text according to these guidelines, making it more polished while preserving its essential message and style.
...
"""
        ...
        response = client.chat.completions.create(
            model="gpt-4o-mini",  # Using the same model as ts1.py
            messages=[
...
def main():
    ...
    try:
        ...
        backup_filename = f"{filename_without_ext}_{timestamp}{os.path.splitext(input_file)[1]}"
    ...
    for idx, segment in enumerate(segments, 1):
        ...
        # Insert the updated content with proper spacing
        inserted_content = '111\n' + updated_content + '\n111'
